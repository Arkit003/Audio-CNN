...
class ResidualBlock(nn.Module):
    def __init__(self,in_channels,out_channels,stride=1):
        ...
        if self.use_shortcut:
            self.shortcut=nn.Sequential(
                #for keeping dimension same we are using 1 kernel(same out put as tensor) in the shrotcut
    ...
    def forward(self,x, fmap_dict = None, prefix = ""):
        ...
        out=self.bn2(out)
        out=torch.relu(out)
        #if x is not dimesionally correct making it correct and then adding
...
class AudioCNN(nn.Module):
    def __init__(self,num_classes=50):
        ...
        self.conv1=nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),#inplace true makes it applied on the last layer
            nn.MaxPool2d(3,2,1)
            )
    ...
    def forward(self,x, return_feature_maps:bool = False):
        if not return_feature_maps:
            ...
            for block in self.layer4:
                x=block(x)
            x=self.avgpool(x)
            x=x.view(x.size(0),-1)#flattening from (B,512,1,1) to (B,512)
            x=self.dropout(x)
